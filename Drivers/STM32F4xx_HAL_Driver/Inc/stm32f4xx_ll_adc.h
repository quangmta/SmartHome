...
#define __LL_ADC_CALC_TEMPERATURE_TYP_PARAMS(__TEMPSENSOR_TYP_AVGSLOPE__,\
  ...
  ((( (                                                                        \
	...
	__STATIC_INLINE uint32_t LL_ADC_REG_GetTriggerSource(ADC_TypeDef *ADCx) {
		uint32_t TriggerSource = READ_BIT(ADCx->CR2,
	 ...
	 *         @arg @ref LL_ADC_REG_FLAG_EOC_UNITARY_CONV
	 */
	__STATIC_INLINE uint32_t LL_ADC_REG_GetFlagEndOfConversion(
			ADC_TypeDef *ADCx) {
	...
	__STATIC_INLINE void LL_ADC_INJ_SetOffset(ADC_TypeDef *ADCx, uint32_t Rank,
		...
		__IO uint32_t *preg = __ADC_PTR_REG_OFFSET(ADCx->JOFR1,
	...
	__STATIC_INLINE void LL_ADC_SetAnalogWDThresholds(ADC_TypeDef *ADCx,
		...
		__IO uint32_t *preg = __ADC_PTR_REG_OFFSET(ADCx->HTR,
	...
	__STATIC_INLINE void LL_ADC_SetMultiDMATransfer(
		...
		MODIFY_REG(ADCxy_COMMON->CCR, ADC_CCR_DMA | ADC_CCR_DDS,
				MultiDMATransfer);
	}
	...
	__STATIC_INLINE uint32_t LL_ADC_INJ_ReadConversionData32(ADC_TypeDef *ADCx,
			uint32_t Rank) {
		__IO uint32_t *preg = __ADC_PTR_REG_OFFSET(ADCx->JDR1,
